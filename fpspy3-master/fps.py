...
class Fingerprint():
    ...
    def enroll(self, idx=None):
        ...
        if not self.start_enroll(idx):
            print("Start enroll failed.")
            return None

        """Start enroll 1, 2, and 3
        """
        for enr_num, enr in enumerate(["enroll1", "enroll2", "enroll3"]):
            print("Taking image {}. ".format(enr_num), end='')
    ...
    def verify(self, idx):
        if not self.capture_finger():
            return False
        if self._send_packet("Verify1_1", idx):
            ack, param, _, _ = self._read_packet()
            if ack:
                return True
            return False
...
if __name__ == '__main__':
    ...
    if f.init():
        ...
        for i in range(3):
            print("Enroll: %s" % str(f.enroll(i + 3)))
            print('Enrolled at ID', i + 3)
            print('Remove finger')
            while f.is_finger_pressed():
                print('.', end='')
                time.sleep(1)
            print()
        ...
        while True:
            try:
                print('Place finger')
                while not f.is_finger_pressed():
                    print('.', end='')
                    time.sleep(1)
                print()
                for i in range(3):
                    print("Verify: %s" % str(f.verify(i + 3)))
                time.sleep(1)
